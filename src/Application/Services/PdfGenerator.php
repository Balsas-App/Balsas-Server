...
class PdfGenerator
{
    ...
    public function generateCheckinsTable(int $boarding): string
    {
        ...
        // Agrupamentos
        $categories = [];   // [category_name => [rows...]] (somente não reembolsados)
        $additional = [];   // rows com add_value > 0 e não reembolsados
        $refunds    = [];   // rows com refunded = 1
        ...
        // Totais
        $baseTotal = 0.0;       // soma dos 'value' por categoria
        $addTotal  = 0.0;       // soma dos 'add_value'
        $refTotal  = 0.0;       // soma dos 'value' a reembolsar
        foreach ($categories as $catRows) {
            foreach ($catRows as $r) $baseTotal += (float)$r['value'];
        }
        foreach ($additional as $r)   $addTotal += (float)$r['add_value'];
        foreach ($refunds as $r)      $refTotal += (float)$r['value'];
